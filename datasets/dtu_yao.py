...
class MVSDataset(Dataset):
    ...
    def read_img(self, filename):
        ...
        return np_img
#     def read_img_stage(self, filename):
#         img = Image.open(filename)
#         # scale 0~255 to 0~1
#         np_img = np.array(img, dtype=np.float32) / 255.
#         #print(np_img.shape)
...
#         return np_img_stage
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            img_aug = self.read_img_aug(img_filename)
            ...
            #imgs_stage.append(img)

        #all
        imgs = np.stack(imgs).transpose([0, 3, 1, 2])
        imgs_aug = np.stack(imgs_aug).transpose([0, 3, 1, 2])
        #ms proj_mats
