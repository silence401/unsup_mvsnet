...
def _bilinear_sample(im, x, y, name='bilinear_sampler'):
    ...
    im_flat = im.permute(0, 2, 3, 1).reshape(-1, channels).float()  # [batch_size * height * width, channels]
    ...
    mask = mask.reshape(batch_size, 1, height, width)
    return output, mask
...
#    print("depth", item["depth"].shape)
    print("cams", item["proj_matrices"]['stage1'].shape)
   ...
   # depth_end = depth_start + (depth_num - 1) * depth_interval
    ...
    ref_cam = torch.tensor(item["proj_matrices"]['stage1'][0]).unsqueeze(0)  # [B, 2, 4, 4]
    # depth
    depth = torch.tensor(item["depth"]['stage1']).unsqueeze(0).squeeze(-1)
    ...
    for view in range(1, view_num):
        view_cam = torch.tensor(item["proj_matrices"]['stage1'][view]).unsqueeze(0)  # [B, 2, 4, 4]
       ...
       # view_image = view_image.permute(0, 2, 3, 1)
        #print(view_image.shape)
        #print("depth:", depth.shape)
        warped_img, mask = inverse_warping(view_image.cuda(), ref_cam.cuda(), view_cam.cuda(), depth.cuda())

        warped_images_np.append(warped_img[0].cpu().numpy())
        view_images_np.append(view_image[0].cpu().numpy())
        masks_np.append(mask.repeat(1, 3, 1, 1)[0].cpu().numpy())
    ...
    plt.subplot(3, 3, 1)
    #print("ref_image_np.shape", ref_image_np.shape)
    ref_image_np = np.transpose(ref_image_np, (1,2,0))

    plt.imshow(ref_image_np)
    plt.title('ref')
    plt.subplot(3, 3, 2)
    view_images_np[0] = np.transpose(view_images_np[0], (1,2,0))
    #plt.imshow(cv2.cvtColor(view_images_np[0], cv2.COLOR_BGR2RGB))
    plt.imshow(view_images_np[0])
    plt.title('src1')
    plt.subplot(3, 3, 3)
    view_images_np[1] = np.transpose(view_images_np[1], (1,2,0))
    plt.imshow(view_images_np[1])
    plt.title('src2')
    plt.subplot(3, 3, 5)
    warped_images_np[0] = np.transpose(warped_images_np[0], (1,2,0))
    masks_np[0] = np.transpose(masks_np[0], (1,2,0))
    ...
    plt.subplot(3, 3, 6)
    warped_images_np[1] = np.transpose(warped_images_np[1], (1,2,0))
    masks_np[1] = np.transpose(masks_np[1], (1,2,0))
