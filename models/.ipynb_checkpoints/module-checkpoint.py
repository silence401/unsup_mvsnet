...
class SaTWNet(nn.Module):
    def __init__(self, base_channels, num_stage, stride=4, arch_mode="fpn"):
        super(SaTWNet, self).__init__()
        assert arch_mode in ["unet", "fpn"], print("mode must be in 'unet' or 'fpn', but get:{}".format(arch_mode))
        ...
        if self.arch_mode == 'unet':
            ...
            final_chs = base_channels * 4
            if num_stage == 3:
                self.inner1 = Conv2d(base_channels * 2, final_chs, 1)
                self.inner2 = Conv2d(base_channels * 1, final_chs, 1)
                ...
                self.inner1 = Conv2d(base_channels * 2, final_chs)

                #self.out2 = nn.Conv2d(final_chs, base_channels, 3, padding=1, bias=False)
                #self.out_channels.append(base_channels)
        #the last layer of conv 
        self.out = ConvBnReLU(base_channels*4, 1)


    def forward(self, features, cams, depth_est):
        """
        features: [num_views {[B C H W]}]
        cams:
        """
       # num_views = len(ref_features['stage1'])
        #the 2st layer output of unet or fpn
        #print("features", features[0]['stage1'].shape)
       # print("cams", cams['stage1'].shape)
        num_views = len(features)
        num_stages = len(features[0])
        #print("num_s", num_stages)
        out = None
        #ref_features = get_stage1(features, num_stages, 0)
        #ref_cams = get_stage2(cams, num_stages, 0)
        for i in range(1, num_views): 
            #src_features = get_stage1(features, num_stages, i)
            #src_cams = get_stage2(cams, num_stages, i)

            ref_fea_conv2 = features[0]['stage1']
            src_fea_conv2 = features[i]['stage1']
            ref_cam_conv2 = cams['stage1'][:, 0]
            src_cam_conv2 = cams['stage1'][:, i]
#             print("ref_fea_conv2.shape", ref_fea_conv2.shape)
#             print("src_fea_conv2.shape", src_fea_conv2.shape)
#             print("ref_cam_conv2.shape", ref_cam_conv2.shape)

            depth_conv2 = depth_est['stage1']
            warped_src_fea, mask = inverse_warping(src_fea_conv2, ref_cam_conv2, src_cam_conv2, depth_conv2)
            fea_diff_conv2 = ref_fea_conv2 - warped_src_fea

            if self.arch_mode == 'unet':
                if self.num_stage == 3:
                    #the 2st layer output of unet or fpn
                    ref_fea_conv1 = ref_features['stage2']
                    src_fea_conv1 = src_features['stage2']
                    ref_cam_conv1 = ref_cams['stage2']
                    src_cam_conv1 = src_cams['stage2']
                    depth_conv1 = depth_est['stage2']
                    warped_src_fea, mask = inverse_warping(src_fea_conv1, ref_cam_conv1, src_cam_conv1, depth_conv1)
                    fea_diff_conv1 = ref_fea_conv1 - warped_src_fea
                    fea_diff_conv1 = self.deconv1(fea_diff_conv1, fea_diff_conv2)

                    ref_fea_conv0 = ref_features['stage3']
                    src_fea_conv0 = src_features['stage3']
                    ref_cam_conv0 = ref_cams['stage3']
                    src_cam_conv0 = src_cams['stage3']
                    depth_conv0 = depth_est['stage3']
                    warped_src_fea, mask = inverse_warping(src_fea_conv0, ref_cam_conv0, src_cam_conv0, depth_conv0)
                    fea_diff_conv0 = ref_fea_conv0 - warped_src_fea
                    fea_diff_conv0 = sef.deconv2(fea_diff_conv0, fea_diff_conv1)

                    output = self.out(fea_diff_conv0)
                    #TODO num_stage == 2
            if self.arch_mode == "fpn":
                if self.num_stage == 3:
                    ref_fea_conv1 = features[0]['stage2']
                    src_fea_conv1 = features[i]['stage2']
                    ref_cam_conv1 = cams['stage2'][:, i]
                    src_cam_conv1 = cams['stage2'][:, i]
                    depth_conv1 = depth_est['stage2']
                    warped_src_fea, mask = inverse_warping(src_fea_conv1, ref_cam_conv1, src_cam_conv1, depth_conv1)
                    fea_diff_conv1 = ref_fea_conv1 - warped_src_fea
                    fea_diff_conv1 = F.interpolate(fea_diff_conv2, scale_factor=2, mode="nearest") + self.inner1(fea_diff_conv1)
                    
                    ref_fea_conv0 = features[0]['stage3']
                    src_fea_conv0 = features[i]['stage3']
                    ref_cam_conv0 = cams['stage3'][:, i]
                    src_cam_conv0 = cams['stage3'][:, i]
                    depth_conv0 = depth_est['stage3']
                    warped_src_fea, mask = inverse_warping(src_fea_conv0, ref_cam_conv0, src_cam_conv0, depth_conv0)
                    fea_diff_conv0 = ref_fea_conv0 - warped_src_fea
                    fea_diff_conv0 = F.interpolate(fea_diff_conv1, scale_factor=2, mode="nearest") +  self.inner2(fea_diff_conv0)
                    
                    #print("fea_diff_conv0", fea_diff_conv0.shape)
                    if out is None:
                        out = self.out(fea_diff_conv0)
                    else:
                        output = self.out(fea_diff_conv0)
                        out = torch.cat((out, output), dim=1)
#         print("weights.shape", out.shape)
#         print("weights.max", torch.max(out))
        #Normalization
        out = F.softmax(out, dim=1)
#         print("weights.shape", out.shape)
       # print("weights.max", torch.max(out))
        #_, _, h, w = output.shape
        #out_max = nn.MaxPool2d((h, w))(output)
...
def get_stage1(origin_dict, num_stages, idx):
    new_dict = {}
    for i in range(num_stages):
        new_dict['stage{}'.format(i+1)] = origin_dict[idx]['stage{}'.format(i+1)]
    return new_dict


def get_stage2(origin_dict, num_stages, idx):
    new_dict = {}
    for i in range(num_stages):
        new_dict['stage{}'.format(i+1)] = origin_dict['stage{}'.format(i+1)][:, idx]
    return new_dict
...
if __name__ == "__main__":
    ...
    num_depth = 48
    dataset = MVSDataset("/userhome/dtu_training", '../lists/dtu/train.txt', 'train',
                         3, num_depth, interval_scale=1.06 * 192 / num_depth)

    dataloader = DataLoader(dataset, batch_size=1)
    item = next(iter(dataloader))
    
    featurenet = FeatureNet(base_channels=8, stride=4, num_stage=3)
    
    imgs = item["imgs"][:, :, :, :, :]  #(B, N, 3, H, W)
    cams = item["proj_matrices"]
    ref_cam = {"stage1": cams["stage1"][:, 0]}
    ref_cam.update({"stage2": cams["stage2"][:, 0]})
    ref_cam.update({"stage3": cams["stage3"][:, 0]})
    
    src_cam = {"stage1": cams["stage1"][:, 1]}
    src_cam.update({"stage2": cams["stage2"][:, 1]})
    src_cam.update({"stage3": cams["stage3"][:, 1]})
    
#     new_depth = {"stage1": ["stage1"]['depth']}
#     new_depth.update({"stage2": origin_dict["stage2"]['depth']})
#     new_depth.update({"stage3": origin_dict["stage3"]['depth']})
    print(item['depth'])
    ...
    satw = SaTWNet(8, 3)
    out = satw(features[0], features[1], ref_cam, src_cam, item['depth'])
